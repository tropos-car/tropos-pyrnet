...
def get_config(config: dict|None = None) -> dict:
    ...
    cfiles = {
        ...
        "file_sazi_mask": "share/pyrnet_mask_sazi.json",
    }
    for fn in cfiles:
...
def to_l1b(
    ...
    ######################################################################################
    ## Mask radiation data for certain solar azimuth angles
    sazi_masks = pyrnet.utils.read_json(config["file_sazi_mask"])
    if config["campaign"] in sazi_masks:
        ...
        for i in range(ds_l1b.station.size):
            ...
            for mask in sazi_masks:
                ...
                for var in radflux_vars:
                    ds_l1b[var].values[msel,i] = np.nan 
